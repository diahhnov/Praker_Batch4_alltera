...
func CreateUserController(c echo.Context) error {
	...
	//render JSON reponse with success message and the new data
...
func UpdateUserController(c echo.Context) error {
	...
	// when the data does not exiat render JSON response with not found message
	return c.JSON(echo.ErrBadRequest.Code, map[string]interface{}{
		"messages": "User Not Found",
	})
}
...
func DeleteUserController(c echo.Context) error {
	...
	// looking for data in index of slice users, then delete element in slice users in slice users and render JSON response
	for i, v := range users {
		if v.Id == id {
